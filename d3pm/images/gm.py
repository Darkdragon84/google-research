...
from d3pm.images import datasets, utils
...
class TrainableModel:
    ...
    def __init__(self, config, dataset=None):
        ...
        if dataset is not None:
            ...
            self.dataset = getattr(datasets, config.dataset.name)(**config.dataset.args)
    ...
    def _gen_samples(self, params, rng, samples_shape):
        ...
        def body_fun(val):
            ...
            return (
                i + 1,
                self.samples_fn(params, jax.random.fold_in(rng, i), samples_shape),
            )
    ...
    def step_fn(self, base_rng, train, state, batch):
        ...
        if train:
            ...
            new_state = state.replace(  # pytype: disable=attribute-error
                step=step + 1, optimizer=new_optimizer, ema_params=new_ema_params
            )
                ...
                ok = jnp.all(
                    jnp.asarray(
                        [
                            ...
                            for p in jax.tree.leaves(new_optimizer)
                        ]
                    )
                )
    ...
    def make_init_state(self):
        ...
        logging_string = (
            f"Param shapes: " f"{jax.tree.map(lambda a: a.shape, init_params)}"
        )
        logging.info(logging_string)
        logging.info(
            "Number of trainable parameters: %d", utils.count_params(init_params)
        )
    ...
    def _calc_eval_metrics(
        self, *, state, eval_iter, eval_steps, eval_base_rng, total_bs
    ):
        ...
        logging.info(
            "Evaluated %d batches (%d examples)",
            len(all_eval_metrics),
        ...
        if all_eval_metrics:
            ...
            warning = (
                f"No eval batches! num_eval={self.dataset.num_eval} "
                f"total_bs={total_bs}"
            )
    ...
    def run_train(self, experiment_dir, work_unit_dir, rng):
        ...
        with metric_writers.ensure_flushes(writer):
            for batch in train_iter:
                ...
                logging.log_first_n(
                    logging.INFO, "Finished training step %d", 5, new_step
                )
                # Log metrics
                if new_step % config.train.log_loss_every_steps == 0:
                    ...
                    metrics["train/steps_per_sec"] = float(
                        config.train.log_loss_every_steps
                ...
                if prev_step == 0 or should_eval:
                    ...
                    samples_to_log = {
                        "eval/samples": self.get_model_samples(
                    ...
                    if samples_to_log:
                        assert all(
                            v.shape == (total_bs, *self.dataset.data_shape)
                    ...
                    if config.train.get("calc_eval_metrics", True):
                        eval_metrics = self._calc_eval_metrics(
                            state=state,
                            eval_iter=eval_iter,
                            eval_steps=config.train.get(
                                "eval_number_steps", self.dataset.num_eval // total_bs
                            ),
                ...
                if config.train.checkpoint_every_secs is not None:
                    ...
                    should_ckpt = (
                        prev_step == 0
                        or new_step == config.train.num_train_steps
                        or should_ckpt
                    )
                else:
                    should_ckpt = False
                ...
                if (
                    ("retain_checkpoint_every_steps" in config.train)
                    and (
                        (new_step % config.train.retain_checkpoint_every_steps == 0)
                        or (new_step == config.train.num_train_steps)
                    )
                    and (jax.host_id() == 0)
                ):
