...
def shard_dataset(dataset, *, shard_id, num_shards):
    ...
    logging.info("Sharding dataset: shard_id=%d num_shards=%d", shard_id, num_shards)
        ...
        *,
        ...
        ds = tfds.load(
            ...
            read_config=(
                None
                if not shuffle
                else tfds.ReadConfig(shuffle_seed=utils.jax_randint(next(global_rng)))
            ),
        )
        if repeat:
            ds = ds.repeat()
        ...
        return self._preprocess_and_batch(ds, batch_shape=batch_shape, augment=augment)
