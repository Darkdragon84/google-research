...
from d3pm.images import entry_point, gm, model, utils
...
class Cifar10DiffusionModel(gm.TrainableModel):
    ...
    def loss_fn(self, rng, train, batch, params):
        ...
        if not train:
            ...
            return loss, {"loss": loss, "prior_bpd": prior_bpd, "total_bpd": total_bpd}
        else:
...
def run_train(*, config, experiment_dir, work_unit_dir, rng):
