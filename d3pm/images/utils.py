...
def normalize_data(x, mode=None):
    if mode is None or mode == "rgb":
        ...
        return 2.0 * normalize_data(x, mode="rgb")
    elif mode == "yuv":
        return (x / 127.5 - 1.0).dot(RGB_TO_YUV)
    else:
...
def sample_categorical(logits, uniform_noise):
    ...
    uniform_noise = jnp.clip(
        uniform_noise, a_min=jnp.finfo(uniform_noise.dtype).tiny, a_max=1.0
    )
...
def categorical_kl_logits(logits1, logits2, eps=1.0e-6):
    ...
    """
    out = jax.nn.softmax(logits1 + eps, axis=-1) * (
...
def global_norm(pytree):
    return jnp.sqrt(
        jnp.sum(jnp.asarray([jnp.sum(jnp.square(x)) for x in jax.tree.leaves(pytree)]))
    )
...
def apply_ema(decay, avg, new):
    return jax.tree.map(lambda a, b: decay * a + (1.0 - decay) * b, avg, new)
...
def tf_to_numpy(tf_batch):
    ...
    return jax.tree.map(lambda x: x._numpy() if hasattr(x, "_numpy") else x, tf_batch)
...
def save_tiled_imgs(filename, imgs, pad_pixels=1, pad_val=255, num_col=0):
    PIL.Image.fromarray(
        np_tile_imgs(imgs, pad_pixels=pad_pixels, pad_val=pad_val, num_col=num_col)
    ).save(filename)
